#!/usr/bin/env python3
"""Fire-and-Forget mixed workload driver for Calm.
...
* Launches multiple reader threads that continuously run analytical queries to observe the impact of concurrent writes.
...
def query_worker(
    ...
    conn = new_mysql_connection(args)
    cursor = conn.cursor()
    sql = f"SELECT COUNT(*), AVG(user_id) FROM {args.table} WHERE ts_ms >= %s"
    window_ms = args.query_window_ms
    completed = 0

    try:
        while not stop_event.is_set():
            threshold = int(time.time() * 1000) - window_ms
            start = time.perf_counter()
...
def main() -> None:
    ...
    print(
        ...
        f" {args.insert_threads} insert threads and {args.query_threads} query threads"
    )
