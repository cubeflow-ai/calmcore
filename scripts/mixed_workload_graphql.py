#!/usr/bin/env python3
"""Fire-and-Forget mixed workload driver for Calm (GraphQL Query Version).
...
* Launches multiple reader threads that continuously run GraphQL queries (instead of SQL SELECT).
...
Example
-------
python scripts/mixed_workload_graphql.py \
...
def query_worker(
    ...
    # 使用 GraphQL query(sql) 执行真正的数据查询
    completed = 0

    try:
        while not stop_event.is_set():
            start = time.perf_counter()
            # GraphQL query: 执行 SQL 查询，相当于 SELECT * FROM table LIMIT 10
            sql = f"SELECT count(*) FROM {args.table} LIMIT 10"
            payload = {
            ...
            if "errors" in data:
                print(f"[query-{worker_id}] GraphQL error: {data['errors']}")
...
def main() -> None:
    ...
    print(
        ...
        f" {args.insert_threads} insert threads and {args.query_threads} query threads (GraphQL queries)"
    )
