...
impl<'a, W: Read + Write + 'a> Drop for RowWriter<'a, W> {
    fn drop(&mut self) {
        if let Err(e) = self.finish_inner(true) {
            // Client disconnected or connection error - this is expected behavior
            // when client closes connection, just log it instead of panicking
            if e.kind() == io::ErrorKind::BrokenPipe
                || e.kind() == io::ErrorKind::ConnectionReset
                || e.kind() == io::ErrorKind::ConnectionAborted
            {
                eprintln!(
                    "ℹ️  [MySQL] Client disconnected during result streaming: {}",
                    e
                );
            } else {
                eprintln!("⚠️  [MySQL] Error finishing row writer: {}", e);
            }
        }
    }
}
