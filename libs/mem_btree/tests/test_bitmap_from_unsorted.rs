...
fn test_unsorted_vec_to_bitmap() {
    ...
    println!(
        "   ✅ from_iter: All {} elements present",
        expected_ids.len()
    );
    ...
    match RoaringBitmap::from_sorted_iter(unsorted_ids.iter().copied()) {
        Ok(bitmap_from_sorted) => {
            println!(
                "   from_sorted_iter result: {} items",
                bitmap_from_sorted.len()
            );
            ...
            if !missing.is_empty() {
                println!(
                    "   ❌ from_sorted_iter: LOST {} elements: {:?}",
                    missing.len(),
                    missing
                );
                ...
                println!(
                    "   ❌ from_sorted_iter: Length mismatch! expected {}, got {}",
                    expected_ids.len(),
                    bitmap_from_sorted.len()
                );
            } else {
        ...
        Err(e) => {
            println!(
                "   ✅ from_sorted_iter correctly rejected unsorted input: {:?}",
                e
            );
        }
    }
