use mem_btree::persist::{ReadSerializer, TreeReader, TreeWriter, UnionLeafSerializer, WriteSerializer};
use mem_btree::BTree;
...
fn benchmark_union_leaf_large_dataset() {
    ...
    println!("  Range查询[10000,10100): {:?} (扫描{}个bitmap值)", small_range_time, count);
    ...
    println!("  Range查询[20000,25000): {:?} (扫描{}个bitmap值)", medium_range_time, count);
    ...
    println!("  Range查询[0,50000): {:?} (扫描{}个bitmap值)", large_range_time, count);
    ...
    println!("  Range查询[10000,10100): {:?} (扫描{}个bitmap值)", small_range_time_union, count);
    ...
    println!("  Range查询[20000,25000): {:?} (扫描{}个bitmap值)", medium_range_time_union, count);
    ...
    println!("  Range查询[0,50000): {:?} (扫描{}个bitmap值)", large_range_time_union, count);
    ...
    if size_overhead >= 0 {
        println!("  额外开销:      {} ({:+.2}%)", format_size(size_overhead as u64), size_overhead_pct);
    } else {
        println!("  节省空间:      {} ({:.2}%)", format_size((-size_overhead) as u64), -size_overhead_pct);
    }
    println!();
    ...
    let write_overhead = write_time_with_union.as_micros() as f64 / write_time_no_union.as_micros() as f64;
    ...
    let point_ratio = point_query_time_union.as_micros() as f64 / point_query_time.as_micros() as f64;
    ...
    println!("    性能比:       {:.2}x", small_range_time_union.as_micros() as f64 / small_range_time.as_micros() as f64);
    println!();
    
    println!("  中范围[5000个key]:");
    println!("    无union_leaf: {:?}", medium_range_time);
    println!("    有union_leaf: {:?}", medium_range_time_union);
    println!("    性能比:       {:.2}x", medium_range_time_union.as_micros() as f64 / medium_range_time.as_micros() as f64);
    println!();
    
    println!("  大范围[50000个key]:");
    println!("    无union_leaf: {:?}", large_range_time);
    println!("    有union_leaf: {:?}", large_range_time_union);
    println!("    性能比:       {:.2}x", large_range_time_union.as_micros() as f64 / large_range_time.as_micros() as f64);
    println!();
...
fn test_union_leaf_correctness() {
    ...
    let mut tree = BTree::new(32);
    for key in 0..5000u32 {  // 增加到5000个键
        let mut bitmap = RoaringBitmap::new();
    ...
    // 验证数据完整性 - 抽样检查
    for key in (0..5000u32).step_by(100) {
        let bitmap = reader.get(&key).expect(&format!("应该能找到key {}", key));
        assert!(bitmap.contains(key), "bitmap应该包含key本身");
        assert!(bitmap.contains(key + 1), "bitmap应该包含key+1");
        assert!(bitmap.contains(key + 2), "bitmap应该包含key+2");
        assert_eq!(bitmap.len(), 3, "每个bitmap应该有3个元素");
    }
