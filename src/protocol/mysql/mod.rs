    ...
    fn on_query(&mut self, query: &str, results: QueryResultWriter<W>) -> io::Result<()> {
        ...
        if query_lower.starts_with("insert") {
            return tokio::runtime::Runtime::new()
                .unwrap()
                .block_on(handle_insert(self.engine.clone(), query, results));
        }
        ...
        if query_lower.starts_with("delete") {
            return tokio::runtime::Runtime::new()
                .unwrap()
                .block_on(handle_delete(self.engine.clone(), query, results));
        }
        ...
        if query_lower.starts_with("select") {
            return tokio::runtime::Runtime::new()
                .unwrap()
                .block_on(execute_query(self.engine.clone(), query, results));
        }

        results.error(
            ErrorKind::ER_NOT_SUPPORTED_YET,
            b"Only SELECT/INSERT/DELETE/CREATE TABLE/DROP TABLE/SHOW TABLES/SHOW DATABASES supported",
        )
    }
}
...
async fn execute_query<W: io::Read + io::Write>(
    ...
    if let Some(partition) = engine.get_partition(0).await {
...
async fn handle_insert<W: io::Read + io::Write>(
    ...
    for (col_name, value) in columns.iter().zip(values.iter()) {
        ...
        let (array, arrow_field) = match field_opt {
            FieldOption::I32 { name, .. } => {
                let val: i32 = value
                    .parse()
                    .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, format!("Invalid I32: {}", e)))?;
                (
            ...
            FieldOption::I64 { name, .. } => {
                let val: i64 = value
                    .parse()
                    .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, format!("Invalid I64: {}", e)))?;
                (
            ...
            FieldOption::U64 { name, .. } => {
                let val: u64 = value
                    .parse()
                    .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, format!("Invalid U64: {}", e)))?;
                (
            ...
            FieldOption::F32 { name, .. } => {
                let val: f32 = value
                    .parse()
                    .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, format!("Invalid F32: {}", e)))?;
                (
            ...
            FieldOption::F64 { name, .. } => {
                let val: f64 = value
                    .parse()
                    .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, format!("Invalid F64: {}", e)))?;
                (
    ...
    let batch = RecordBatch::try_new(schema, arrays)
        .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to create batch: {}", e)))?;
    ...
    let pk_field = meta
        ...
        .primary_key
        .as_ref()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "Table has no primary key"))?;
    ...
    let partition_id = engine
        ...
        .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Partition routing failed: {}", e)))?;
    
    // 获取 partition 并插入
    let partition = engine
        .get_partition(partition_id as u64)
        .await
...
async fn handle_delete<W: io::Read + io::Write>(
    ...
    if let Some(partition) = engine.get_partition(0).await {
        ...
        ctx.register_table(&table_name, provider)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Register table failed: {}", e)))?;
    } else {
    ...
    let df = ctx
        .sql(&select_query)
        .await
        .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, format!("Query parse error: {}", e)))?;
