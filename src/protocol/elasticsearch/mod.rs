...
use crate::{
    calm::{CalmRpcService, CalmService},
...
async fn create_index(
    ...
    // 使用 CalmRpcService 创建表
    CalmRpcService::create_table(
        Arc::as_ref(&server.calm_service).clone(),
...
async fn get_index(
    ...
    for field in &meta.table.schema.fields {
    ...
    Ok(poem::web::Json(serde_json::json!({
        index: {
            ...
            "settings": {
                "index": {
                    "number_of_shards": meta.partitions.read().await.len(),
...
async fn list_indices(
    ...
    let tables = server.calm_service.catalog().list_tables().await;
...
async fn bulk_operation_impl(
    ...
    // 按索引分组文档: index_name -> vec<(doc_id, doc)>
    let mut docs_by_index: HashMap<String, Vec<(String, Value)>> = HashMap::new();
...
async fn handle_aggregation_search(
    ...
    let mut count_stream = server
        .calm_service
        .execute_query_stream(&count_sql)
        .await
...
async fn search_impl(
    ...
    if let Some(aggregations) = aggs {
        ...
        let schema_arc = Arc::new(meta.table.schema.clone());
    ...
    if let Some(query) = &search_req.query {
        ...
        if let Some(where_sql) = convert_es_query_to_sql(query, &meta.table.schema) {
            where_clause = format!(" WHERE {}", where_sql);
    ...
    // 执行实际查询
    let mut search_stream = match server.calm_service.execute_query_stream(&sql).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let response_body = serde_json::json!({
        ...
        "timed_out": false,
        "_shards": {
            "total": meta.partitions.read().await.len(),
            "successful": meta.partitions.read().await.len(),
            "skipped": 0,
            "failed": 0
        },
        "hits": {
...
fn convert_es_query_to_sql(query: &Value, schema: &crate::schema::Schema) -> Option<String> {
    use crate::protocol::elasticsearch::query_rewriter::QueryRewriter;
