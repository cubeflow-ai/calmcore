...
impl TopKMerger {
    ...
    /// # Returns
    /// 全局 TOP-K 结果
    pub fn merge(&self, partition_results: Vec<Vec<RecordBatch>>) -> DFResult<Vec<RecordBatch>> {
    ...
    fn apply_offset_limit(&self, batch: RecordBatch) -> DFResult<RecordBatch> {
        ...
        if start >= end {
            // 没有数据
            return RecordBatch::try_new(batch.schema(), vec![])
                .map_err(|e| datafusion::error::DataFusionError::ArrowError(Box::new(e), None));
        }
...
fn concat_batches(schema: &SchemaRef, batches: &[RecordBatch]) -> DFResult<RecordBatch> {
    ...
    arrow_concat(schema, batches).map_err(|e| {
