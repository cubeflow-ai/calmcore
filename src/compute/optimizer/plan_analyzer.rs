...
fn analyze_sort_limit(sql: &str) -> Option<SortLimitInfo> {
    ...
    // 检查是否有 ORDER BY
    let has_order_by = sql.contains("order by");
    ...
    if has_order_by {
        ...
        // 纯 LIMIT 查询（无 ORDER BY）
        Some(SortLimitInfo {
            sort_fields: vec![], // 空排序字段
            limit,
            offset,
            can_use_index: false,
...
fn extract_sort_fields(sql: &str) -> Option<Vec<(String, bool)>> {
    ...
    // 🔧 修复：使用 \blimit\b 匹配完整单词，使用非贪婪匹配 .+?
    let re = Regex::new(r"order\s+by\s+(.+?)(?:\s+limit\b|$)").ok()?;
    let caps = re.captures(sql)?;
