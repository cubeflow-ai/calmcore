...
def scan_parquet_files(data_dir, min_age_minutes=20):
    ...
    for root, dirs, files in os.walk(data_dir):
        # 获取相对于 data_dir 的路径作为 partition_id
    ...
    print(f"  Skipped {skipped_recent} recent files (< {min_age_minutes} minutes old)\n")
    
    # 按 partition_id 和文件名排序
    parquet_files.sort(key=lambda x: (x['partition_id'], x['filename']))
    
    return parquet_files

def check_segment_exists(graphql_url, table, partition_id):
    """
    检查 partition 是否已经有 segments
    
    Returns:
        int: segment 数量，-1 表示出错
    """
    query = """
    query GetTableInfo($table: String!) {
        table(name: $table) {
            name
            partitionCount
        }
    ...
    try:
        response = requests.post(
            graphql_url,
            json={'query': query, 'variables': {'table': table}},
            timeout=10
        )
        
        if response.status_code == 200:
            result = response.json()
            if 'data' in result and result['data'].get('table'):
                # 表存在，但我们无法直接查询 segment 数量
                # 返回 0 表示可以继续（假设没有重复）
                return 0
            return -1
        return -1
...
def load_segment_graphql(graphql_url, table, partition_id, file_path):
    """
    ...
    """
    mutation = """
    mutation LoadSegmentFromFile($table: String!, $partitionId: String!, $filePath: String!) {
        loadSegment(input: {
            table: $table
...
Examples:
  ...
  python3 load_parquet_segments.py --table mytable --data-dir /other/path --load
    ...
    parser.add_argument('--data-dir', default='/mnt/nvme_cfs/r2data/data',
                        help='Data directory to scan (default: /mnt/nvme_cfs/r2data/data)')
    parser.add_argument('--table', default='r2api', 
                        help='Table name (default: r2api)')
    parser.add_argument('--graphql-url', default='http://127.0.0.1:9567/graphql', 
    ...
    if not args.load:
        print_colored(Colors.YELLOW, "Mode: PREVIEW ONLY (use --load to actually load segments)")
    else:
    ...
    for file_info in parquet_files[:10]:  # 只显示前 10 个
        print(f"{file_info['partition_id']:<15} {file_info['filename']:<40} {file_info['age_minutes']:<12.1f}")
    ...
    # 预览模式
    if not args.load:
        print_colored(Colors.YELLOW, "🔍 Preview complete. Use --load to actually load these segments.")
        return 0
    ...
    for idx, file_info in enumerate(parquet_files, 1):
        partition_id = file_info['partition_id']
        file_path = file_info['file_path']
        filename = file_info['filename']
        
        print(f"[{idx}/{len(parquet_files)}] Loading {partition_id}/{filename}...", end=' ')
        ...
        if idx % 10 == 0:
            ...
            print(f"  Progress: {idx}/{len(parquet_files)} files, {rate:.1f} files/sec, {total_docs:,} docs loaded\n")
    ...
    if failed_count == 0:
        print_colored(Colors.GREEN, "\n🎉 All segments loaded successfully!")
        return 0
    else:
