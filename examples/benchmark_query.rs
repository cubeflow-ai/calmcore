use calm::{
    partition::Partition,
    schema::{
        compute::PartitionTableProvider,
        field::FieldOption,
        Schema,
    },
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 检查是否已有持久化数据
    let calm_has_data = calm_data_dir.exists() && 
        std::fs::read_dir(&calm_data_dir)?.count() > 0;
    ...
    if calm_has_data && parquet_has_data {
        ...
        println!("✓ Partition loaded: {} frozen segments\n", partition.frozen_count());
    } else {
        ...
        // 生成数据
        let ids: Vec<i64> = (start_id..(start_id + batch_size)).map(|i| i as i64 + 1).collect();
        ...
        let batch = RecordBatch::try_new(
            ...
            if (batch_idx + 1) % 100 == 0 {
                println!("  Inserted {} / {} batches ({} records)", 
                    batch_idx + 1, num_batches, (batch_idx + 1) * batch_size);
            }
        }
        ...
        if !calm_has_data {
            ...
            println!("  Calm Partition current segment doc_count: {}", current_count);
            ...
            println!("  ✓ All segments persisted in {:.2?}", persist_start.elapsed());
        ...
        println!("  ✓ Partition loaded from disk in {:.2?}", load_start.elapsed());
        ...
        if !parquet_has_data {
            ...
            for batch_idx in 0..num_batches {
                ...
                if (batch_idx + 1) % 10 == 0 {
                    println!("  Written {} / {} batches to Parquet", batch_idx + 1, num_batches);
                }
            }
    ...
    let queries = vec![
        ...
        ("Q4: Range query (value >= 500)", 
         "SELECT COUNT(*) as total FROM {} WHERE value >= 500"),
        
        ("Q5: Complex filter (category = 'B' AND value < 300)", 
         "SELECT COUNT(*) as total FROM {} WHERE category = 'B' AND value < 300"),
        
        ("Q6: Aggregation with filter", 
         "SELECT category, COUNT(*) as cnt, AVG(value) as avg_val FROM {} WHERE value >= 100 GROUP BY category"),
        
        ("Q7: Range query (timestamp BETWEEN)", 
         "SELECT COUNT(*) as total FROM {} WHERE timestamp BETWEEN 1700500000 AND 1700600000"),
        
        ("Q8: Multiple conditions (OR)", 
         "SELECT COUNT(*) as total FROM {} WHERE category = 'A' OR category = 'C'"),
    ];

    // 6. 运行性能对比
    println!("{:<50} {:>15} {:>15} {:>10}", "Query", "Calm (ms)", "Native (ms)", "Speedup");
    println!("{}", "=".repeat(92));
    ...
    for (query_name, query_template) in queries {
        ...
        println!("{:<50} {:>15.2} {:>15.2} {:>10}", 
            query_name, calm_avg, native_avg, speedup_str);
        
        // 打印详细统计信息
        println!("  Calm: {} batches, {} rows | Native: {} batches, {} rows", 
            calm_result_batches.len(), calm_total_rows,
            native_result_batches.len(), native_total_rows);
        
        // 校验结果行数是否一致
        if calm_total_rows != native_total_rows {
            println!("  ⚠️  WARNING: Row count mismatch! Calm={}, Native={}", 
