use calmcore::{util::CoreResult, *};
use proto::core::{
    field::{self, embedding_option::Metric, EmbeddingOption},
    Schema,
};
fn main() -> CoreResult<()> {
    let schema_name = "test";
    let data_path = "/tmp/calm_test";

    let _ = std::fs::remove_dir_all(data_path);

    // to create schema and space
    let engine = core.create_engine(make_schema(schema_name))?;

    engine.mutate_json(
        ActionType::Upsert,
        "1".to_string(),
        br#"{"name":"hello" , "age":32 , "sex":true, "skill":[1.0, 2.1,2.2]}"#,
    )?;

    engine.mutate_json(
        ActionType::Upsert,
        "2".to_string(),
        br#"{"name":"hello1" , "age":22 , "sex":false, "skill":[1.0, 5.1,2.2]}"#,
    )?;

    let resp =
        engine.sql("select * from test where age > 20 query skill='[1.0, 5.1,2.2]' limit 20")?;

    println!("resp: {:?}", resp);

    Ok(())
}

fn make_schema(schema_name: &str) -> Schema {
    let mut fields = std::collections::HashMap::new();

    let name = String::from("name");
    fields.insert(
        name.clone(),
        proto::core::Field {
            name,
            r#type: proto::core::field::Type::String as i32,
            option: None,
        },
    );

    let name = String::from("age");
    fields.insert(
        name.clone(),
        proto::core::Field {
            name,
            r#type: proto::core::field::Type::Int as i32,
            option: None,
        },
    );

    let name = String::from("skill");
    fields.insert(
        name.clone(),
        proto::core::Field {
            name,
            r#type: proto::core::field::Type::Vector as i32,
            option: Some(field::Option::Embedding(EmbeddingOption {
                embedding: String::from("no"),
                dimension: 3,
                metric: Metric::Euclidean as i32,
                batch_size: 1000,
            })),
        },
    );

    Schema {
        name: String::from(schema_name),
        id: 1,
        fields,
        metadata: None,
        schemaless: false,
    }
}
