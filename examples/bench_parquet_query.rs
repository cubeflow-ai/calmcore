/// Benchmark: TreeReader vs Pure Parquet 查询性能对比
///
/// 测试已有数据文件的查询性能:
/// 1. TreeReader: 从 json_upsert_demo_data 读取
/// 2. Pure Parquet: 创建单个 Parquet 文件
...
// Parquet 查询测试
fn bench_parquet_query(path: &str, total_rows: usize, batch_size: usize, query_count: usize) {
    println!("\n🟢 测试 Parquet 查询性能");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
...
fn main() {
    ...
    println!("   Batch/RowGroup 大小: {}", batch_size);
    println!("   查询次数: {}", query_count);

    let parquet_path = "/tmp/bench_pure_parquet.parquet";

    // 创建 Parquet 文件
    create_parquet_file(parquet_path, total_rows, batch_size);

    // 测试查询性能
    bench_parquet_query(parquet_path, total_rows, batch_size, query_count);

    println!("\n════════════════════════════════════════════");
    println!("✨ 测试完成!");
    println!("\n💡 说明:");
    println!("   - 每次查询都重新读取文件(模拟无缓存场景)");
    println!("   - 利用 Row Group 索引直接定位目标批次");
    println!("   - 与 TreeReader 的 BTree 索引性能可对比");
}
